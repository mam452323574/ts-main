...
import { parseSocialRecordPostViewsRequest } from '../_shared/phase2Contracts.ts';
...
import type { SocialRecordPostViewsResponse } from '../_shared/phase2Types.ts';
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    requireFeatureEnabled(
      ...
      'social_disabled',
      'Social unique post views are currently disabled',
    );

    const requestBody = parseSocialRecordPostViewsRequest(
    ...
    if (visiblePostsError) {
      throw new Phase2HttpError(
        500,
        'social_post_view_lookup_failed',
        'Failed to validate visible posts for unique views',
      );
    }
    ...
    const visiblePostIds = (Array.isArray(visiblePosts) ? visiblePosts : [])
      .filter(
        (post) =>
          ...
          (post.moderation_state === 'approved' || post.author_id === user.id),
      )
      .map((post) => post.id)
      .filter((postId): postId is string => typeof postId === 'string' && postId.length > 0);

    if (visiblePostIds.length === 0) {
      const responseBody: SocialRecordPostViewsResponse = {
        success: true,
        recorded_count: 0,
      };
    ...
    const { data, error } = await supabase.rpc('record_social_post_views', {
    ...
    if (error) {
      throw new Phase2HttpError(
        500,
        'social_post_view_record_failed',
        'Failed to record unique post views',
      );
    }
    ...
    logPhase2Error('[social-record-post-views] Request failed', error, {
