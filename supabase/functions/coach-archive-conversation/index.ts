...
import { archiveCoachConversation } from '../_shared/coachConversationQuota.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const result = await archiveCoachConversation(supabase, {
      conversationId,
      userId: user.id,
    });
    ...
    logPhase2Error('[coach-archive-conversation] Request failed', error, {
      request_id: requestId,
    });
