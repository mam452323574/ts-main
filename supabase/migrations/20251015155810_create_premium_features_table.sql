/*
  ...
  Allows A/B testing, promotions, and feature gating without code changes.

  ## New Tables

  ### premium_features
  - `id` (uuid, primary key) - Unique identifier
  - `feature_key` (text, UNIQUE, NOT NULL) - Programmatic identifier (e.g., 'advanced_recipes')
  ...
  - `category` (text) - Feature category (e.g., 'content', 'analytics', 'scans')
  - `created_at` (timestamptz) - When feature was added
  - `updated_at` (timestamptz) - Last modification time
  - `enabled` (boolean, DEFAULT true) - Whether feature is active at all

  ## Features
  - Dynamic feature gating without app redeployment
  - Support for temporary promotions (toggle requires_premium)
  - Category-based organization for bulk operations
  - Complete audit trail of feature configuration changes
  - Fast lookups via indexed feature_key

  ## Security
  - Enable RLS on premium_features table
  - All authenticated users can read features for client-side gating
  - Only admins can modify feature configuration (future admin panel)
  - Public read access for feature discovery

  ## Initial Data
  - Seeds core premium features for the Health Scan app
  - Advanced recipes, detailed analytics, unlimited scans
*/
...
INSERT INTO premium_features (
  ...
  (
    'unlimited_scans',
    'Scans Illimités',
    'Nombre illimité de scans corporels, santé et nutrition par jour',
    true,
    'scans',
    true
  ),
  (
    'premium_exercises',
    'Exercices Premium',
    'Programmes d''exercices personnalisés et vidéos HD',
    true,
    'content',
    true
  ),
  (
    'export_data',
    'Export de Données',
    'Export de vos données de santé en PDF ou CSV',
    true,
    'analytics',
    true
  ),
  (
    'priority_support',
    'Support Prioritaire',
    'Réponses rapides de notre équipe support',
    true,
    'support',
    true
  ),
  ...
  (
    ...
    'Planification automatique des repas selon vos objectifs',
    true,
    'content',
    true
  )
...
BEGIN
  ...
  -- If feature doesn't exist or doesn't require premium, grant access
