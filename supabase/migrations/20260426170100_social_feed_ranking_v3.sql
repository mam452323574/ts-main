-- Social feed ranking v3
-- - Adds language_match_boost and country_match_boost on top of v2 score.
-- - Boosts are additive, NULL-safe, and capped (+10.5 max combined).
-- - On a fully legacy dataset (language_code IS NULL everywhere), v3 score
--   equals v2 score exactly: zero regression risk.
-- - Viewer language/country are resolved hybrid: client-supplied params win,
--   otherwise fall back to user_profiles via auth.uid() (single SELECT).

CREATE OR REPLACE FUNCTION public.calculate_social_post_rank_v3(
...
AS $$
DECLARE
  -- v2 weights, kept identical to preserve baseline behaviour.
  ...
  -- v3 locale boosts. Sized to be tangible without dominating engagement:
  -- combined max +10.5 < freshness_decay at 1h (~16). A viral cross-language
  -- post still wins comfortably; matched-language posts get a fair tiebreak.
...
BEGIN
  ...
  -- Locale boosts: NULL on either side => 0 (neutral, never punitive).
  -- Format normalization is enforced by CHECK constraints on the columns
  -- and by the RPC's defensive lower()/upper() pass.
  language_boost := CASE
...
COMMENT ON FUNCTION public.calculate_social_post_rank_v3(
  timestamptz, integer, integer, integer, integer, integer, integer,
  text, text, text, text
) IS 'Social ranking v3 = v2 baseline + additive language/country boosts (NULL-safe, capped at +10.5).';

-- Drop the prior 3-arg signature so we can extend RETURNS TABLE.
...
BEGIN
  ...
  -- viewer field is missing AND the request is authenticated. This
  -- short-circuits the lookup when the client already passes both.
  ...
  -- Defense in depth: re-normalize to expected format. Mirrors the
  -- CHECK constraints. Anything off-spec falls back to NULL (neutral).
