-- Social feed Phase D — robustness improvements (D1, D3, D4, D5)
--
--   D1. last_engagement_at column + triggers on social_post_likes/comments.
--       calculate_social_post_rank_v7 multiplies engagement signals by
--       exp(-(now - last_engagement_at)/24h) so a stale post with old likes
--       can't dominate a fresh post anymore.
--
--   D3. Per-(viewer, post) exploration seed: a deterministic noise in
--       [-0.5, +0.5] is added to every score, breaking ties and ensuring
--       light-but-permanent exploration. Avoids hard rank lock-in.
--
--   D4. author_post_sequence_snapshot column + BEFORE INSERT trigger.
--       Replaces the O(N) CTE used by the prior RPC. Includes backfill.
--
--   D5. Log-saturated engagement signals: replace `likes * 3` by
--       `5 * ln(1 + likes)` (idem dislikes, distinct commenters). Caps
--       virality so a single viral post can't crowd out a whole feed.
--
-- Compatibility:
--   - v6 function is kept in place for rollback.
--   - get_social_feed_page keeps the same 5-arg signature (text, integer,
--     integer, text, text). Cursor stays offset-based. D2 (keyset cursor)
--     is delivered in a separate migration after the client build that
--     ships the composite-cursor parser.

-- ---------------------------------------------------------------------------
-- 1. Schema additions (D1, D4)
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_social_posts_last_engagement_at
  ON public.social_posts (last_engagement_at DESC)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_social_posts_author_seq_snapshot
  ON public.social_posts (author_id, author_post_sequence_snapshot)
  WHERE deleted_at IS NULL;

-- ---------------------------------------------------------------------------
-- 2. Backfill (idempotent — only fills NULLs)
-- ---------------------------------------------------------------------------

UPDATE public.social_posts AS post
   SET last_engagement_at = GREATEST(
     post.created_at,
     COALESCE(
       (SELECT MAX(r.updated_at)
          FROM public.social_post_likes r
...
UPDATE public.social_posts AS post
   SET author_post_sequence_snapshot = numbered.seq
  FROM numbered
 WHERE numbered.id = post.id
   AND post.author_post_sequence_snapshot IS NULL;

-- ---------------------------------------------------------------------------
-- 3. Triggers (D1, D4)
-- ---------------------------------------------------------------------------

-- D1: bump last_engagement_at on every like / dislike change
...
DROP TRIGGER IF EXISTS tg_social_post_likes_last_engagement
  ON public.social_post_likes;
CREATE TRIGGER tg_social_post_likes_last_engagement
  AFTER INSERT OR UPDATE OR DELETE ON public.social_post_likes
  FOR EACH ROW EXECUTE FUNCTION public.handle_social_post_last_engagement_from_like();

-- D1: bump last_engagement_at when a comment becomes (or stops being) visible
...
DROP TRIGGER IF EXISTS tg_social_comments_last_engagement
  ON public.social_comments;
CREATE TRIGGER tg_social_comments_last_engagement
  AFTER INSERT OR UPDATE OR DELETE ON public.social_comments
  FOR EACH ROW EXECUTE FUNCTION public.handle_social_post_last_engagement_from_comment();

-- D4: assign author_post_sequence_snapshot on insert
CREATE OR REPLACE FUNCTION public.assign_social_post_author_sequence()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NEW.author_post_sequence_snapshot IS NULL THEN
    NEW.author_post_sequence_snapshot := COALESCE(
      (
        SELECT MAX(author_post_sequence_snapshot)
          FROM public.social_posts
  ...
  END IF;
  IF NEW.last_engagement_at IS NULL THEN
    NEW.last_engagement_at := NEW.created_at;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS tg_social_posts_assign_author_sequence
  ON public.social_posts;
CREATE TRIGGER tg_social_posts_assign_author_sequence
  BEFORE INSERT ON public.social_posts
  FOR EACH ROW EXECUTE FUNCTION public.assign_social_post_author_sequence();

-- ---------------------------------------------------------------------------
-- 4. Scoring v7 — v6 baseline + D1 freshness factor + D5 log saturation + D3
-- ---------------------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION public.calculate_social_post_rank_v7(
  ...
  p_last_engagement_at timestamptz,
  p_exploration_seed numeric
)
RETURNS numeric
LANGUAGE plpgsql
STABLE
...
DECLARE
  ...
  log_like_weight constant numeric := 5.0;
  log_dislike_weight constant numeric := 6.0;
  log_commenter_weight constant numeric := 4.0;
  ...
  engagement_freshness_tau_hours constant numeric := 24.0;
  exploration_amplitude constant numeric := 0.5;

  age_hours numeric := GREATEST(
    EXTRACT(EPOCH FROM (now()::timestamptz - p_created_at)) / 3600.0,
    0
  );
  engagement_age_hours numeric;
  engagement_freshness numeric;
  avg_dwell_ms numeric;
  laugh_wow_count numeric;
  total_reactions numeric;
  freshness_decay numeric;
  log_likes numeric;
  log_dislikes numeric;
  log_commenters numeric;
  engagement numeric;
  ...
  exploration_noise numeric;
BEGIN
  freshness_decay := freshness_numerator / (age_hours + freshness_offset);

  -- D1: dampen engagement by recency. Time since last_engagement_at, capped to
  -- 0 (positive) so live engagement keeps the post hot.
  IF p_last_engagement_at IS NULL THEN
    engagement_freshness := 1.0;
  ELSE
    engagement_age_hours := GREATEST(
      EXTRACT(EPOCH FROM (now()::timestamptz - p_last_engagement_at)) / 3600.0,
      0
    );
    engagement_freshness := EXP(- engagement_age_hours / engagement_freshness_tau_hours);
  END IF;

  -- D5: log-saturated engagement. Each signal scales as 5 * ln(1+N), capping
  -- the marginal contribution of additional reactions. Dislikes still cost
  -- ~20% more than likes pay.
  log_likes := LN(1 + GREATEST(COALESCE(p_effective_like_count, 0), 0));
  log_dislikes := LN(1 + GREATEST(COALESCE(p_effective_dislike_count, 0), 0));
  log_commenters := LN(1 + GREATEST(COALESCE(p_distinct_commenter_count, 0), 0));

  engagement := (
    log_likes * log_like_weight
    - log_dislikes * log_dislike_weight
    + log_commenters * log_commenter_weight
  ...
  -- D3: exploration. Caller passes a per-(viewer, post) seed in [-1, +1].
  -- We scale to a bounded amplitude so it can break ties without overpowering
  -- real signals.
  IF p_exploration_seed IS NULL THEN
    exploration_noise := 0.0;
  ELSE
    exploration_noise := LEAST(1.0, GREATEST(-1.0, p_exploration_seed))
      * exploration_amplitude;
  END IF;
...
COMMENT ON FUNCTION public.calculate_social_post_rank_v7(
  timestamptz, integer, integer, integer, integer, integer, integer,
  text, text, text, text, text, integer, bigint, integer, integer, jsonb,
  timestamptz, numeric
) IS 'Social ranking v7 = v6 with D1 engagement_freshness, D5 log-saturated engagement, D3 exploration noise.';

-- ---------------------------------------------------------------------------
-- 5. RPC get_social_feed_page — same offset-based signature, uses v7
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  public_posts AS (
    SELECT
      ...
      -- D4: read snapshot column directly, no CTE needed
      COALESCE(post.author_post_sequence_snapshot, 2147483647) AS author_post_sequence,
      -- D3: deterministic exploration seed per (viewer, post)
      (
